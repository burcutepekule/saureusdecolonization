...
if(!an.error.occured){
  out_for=tryCatch({ode(
    ...
    y       = ystart_for,
    times   = c(times_pre[length(times_pre)],times_for[times_for<(time_trt_end+320*dt)]),
    parms   = params,
    method  = solverMethod,
...
if(!an.error.occured){
  ystart_cut = unlist(out_for[dim(out_for)[1],2:dim(out_for)[2]])
}
...
if(!an.error.occured){
  ...
  out[out<(1/popSize)]=0
  out_all_final      = out
  out_all_probiotic = out_all_final
